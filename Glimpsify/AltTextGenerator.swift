...
class AltTextGenerator {
  ...
  private func callVisionAPI(base64Image: String) async throws -> String {
    ...
    case .openAI:
      return try await callOpenAIAPI(base64Image: base64Image, apiKey: apiKey)
    case .claude:
      return try await callClaudeAPI(base64Image: base64Image, apiKey: apiKey)
    }
  }
  ...
  // MARK: - OpenAI API
  private func callOpenAIAPI(base64Image: String, apiKey: String) async throws -> String {
  ...
  // MARK: - Claude API
  private func callClaudeAPI(base64Image: String, apiKey: String) async throws -> String {
    ...
    return apiResponse.content.first?.text?.trimmingCharacters(in: .whitespacesAndNewlines)
